...
This short book is a concise introduction to data analysis meant to address some of these problems.
In it I try to explain and apply some core concepts that will, I hope, make data analysis easier and more useful for students in engineering or a related field of science.
...
Spreadsheets do not facilite repeating or checking any of the steps listed above, but in particular, data manipulation.
...
Here I am I also mean \textit{inferential} statistics, i.e., methods and models meant to make some kind of inference.
...
To help understand why, it can be useful to think about some basic concepts in inferential statistics.
...
Our mean values in this case are \Sexpr{signif(mean(subset(d, x == 'Reference')$y), 3)} (reference) and \Sexpr{signif(mean(subset(d, x == 'Super treatment')$y), 3)} (the new treatment).
...
It does not have a mean of zero, is repeatable, and it may be possible to assign it to a particular cause.
...
\begin{enumerate}
  ...
  \item What type of a relationship do you expect (or see) between your treatments and the response variable(s)? How can your approach to data analysis accommodate this?
\end{enumerate}
