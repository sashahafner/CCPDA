...
In Python, \textt{describe()} can be used.
...
\begin{figure}
\includegraphics[width = 120mm]{images/rstudio1.png}
...
To keep the risk of a Type I error low, it is good practice to avoid unnecessary hypothesis tests.
This is why I recommend minimizing the use of so-called multiple comparison tests, such as Tukey's HSD test.
It is true that the HSD test maintains a low experiment-wise error rate, but this is at the expense of power. 
Instead, try to focus on the most important comparisons with e.g., Dunnett's or Bonferroni's adjustments \citep{belloInvitedReviewReproducible2018,zarBiostatisticalAnalysis1999}.
If appropriate, treat your factor levels as a single quantitative variable and use regression.
...
\begin{enumerate}
  \item What question would you like you to answer? Is your sample appropriate for your question? The sample should reflect the population that you are interested in.
...
The response variable is \texttt{y}.
...
We can exclude them but then we should also add the number of measurements.
...
This approach (paired measurements or repeated measures) is more \textit{powerful} than the use of separate units of observation, because some of the random error is eliminated.
Use this fact to your advantage if you can.
...
\begin{numbox}
  ...
  First, it makes the effects of predictor variables \textit{multiplicative}, so a fixed absolute change in the predictor causes (or is correlated with) a relative change in the response.
  Second, it changes the assumed error distribution from normal to lognormal.
  In many cases both are needed together, conveniently.\footnote{An alternative is generalized linear models, where the error distribution can be specified separately from the transformation, or, in that case, the link function.\citep{glm}}.
\end{numbox}
\end{minipage}}
